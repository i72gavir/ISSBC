...
class PDFManagerWindow(QDialog):
    ...
    def add_pdf(self):
        ...
        for f in files:
            if f not in self.model.pdfs:
                self.model.pdfs.append(f)
                added = True
        ...
        if not added:
            QMessageBox.information(
                self,
                "Información",
                "Los PDFs seleccionados ya están añadidos."
            )
    ...
    def remove_pdf(self):
        ...
        for item in selected_items:
            pdf_path = item.text()
            if pdf_path in self.model.pdfs:
                self.model.pdfs.remove(pdf_path)
